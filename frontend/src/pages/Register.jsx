  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50">
      <div className="max-w-md w-full bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 text-center">
        ...
        <p className="text-slate-400 mb-8 font-medium">Create your professional workspace</p>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <input type="email" placeholder="Email Address" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} required />
        ...
        <p className="mt-8 text-sm text-slate-400">Already have an account? <span onClick={toggleAuth} className="text-indigo-600 font-bold cursor-pointer underline">Login here</span></p>
      </div>
    </div>
  );
};
